...
import {config, getRandomBalance, getWalletId} from '../config-insufficient.js';
...
export default function () {
    console.info(`🌱 Seeding ${config.WALLET_POOL_SIZE} wallets for insufficient balance tests...`);
    ...
    for (let i = 1; i <= config.WALLET_POOL_SIZE; i++) {
        ...
        const payload = JSON.stringify({
            owner: `insufficient-user-${String(i).padStart(3, '0')}`,
        ...
        sleep(0.05); // Small sleep to avoid overwhelming the server during setup
    }

    console.info(`🎯 Insufficient balance suite seeding completed: ${createdWallets} wallets created, ${failedWallets} failed`);
