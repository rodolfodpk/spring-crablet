    ...
    public static StoredEvent createEvent(WalletEvent walletEvent) {
        try {
            ...
            List<Tag> tags = switch (walletEvent) {
                case com.crablet.wallet.domain.event.WalletOpened wo -> List.of(new Tag("wallet_id", wo.walletId()));
                case com.crablet.wallet.domain.event.MoneyTransferred mt -> List.of(
                        new Tag("transfer_id", mt.transferId()),
                        new Tag("from_wallet_id", mt.fromWalletId()),
                        new Tag("to_wallet_id", mt.toWalletId())
                );
                case com.crablet.wallet.domain.event.DepositMade dm -> List.of(new Tag("wallet_id", dm.walletId()));
                case com.crablet.wallet.domain.event.WithdrawalMade wm -> List.of(new Tag("wallet_id", wm.walletId()));
                default -> List.of();
            };
