...
See [EventStore Testing](../crablet-eventstore/TESTING.md) and the `/crablet-test-authoring` skill for the complete testing guide.
...
public class WalletService {
    ...
    // With a caller-provided command ID for safe retries:
    public ExecutionResult deposit(String walletId, String depositId, int amount,
                                   UUID commandId) {
        DepositCommand command = DepositCommand.of(depositId, walletId, amount, "Salary");
        return commandExecutor.execute(command, CommandExecutionOptions.builder()
                .commandId(commandId)
                .build());
    }
}
...
eventStore.executeInTransaction(txStore -> {
    ...
    // 3. Store command (uses same transaction; transaction_id is captured by pg_current_xact_id())
    if (txStore instanceof CommandAuditStore auditStore)
        auditStore.storeCommand(commandJson, commandType, commandId, occurredAt);
...
5. **Command Storage**: `txStore` is cast to `CommandAuditStore`; `storeCommand()` uses the same connection — `transaction_id` is captured by `pg_current_xact_id()` in SQL, linking the command row to its events
6. **Atomic Commit**: All operations commit together, or all rollback on any error

### Transaction ID

The `transactionId` is Crablet's PostgreSQL event-store transaction ID. It is generated by PostgreSQL's `pg_current_xact_id()` function when events are appended:
...
**Important points:**
- **Same event-store transaction ID**: All events appended in one command execution have the same `transaction_id`, and the command audit row uses that same value.
- **Generated on append**: The `transactionId` is only available after the append method returns it.
- **Represents the database transaction**: `pg_current_xact_id()` returns the ID of the current PostgreSQL transaction.
- **Not a business transaction ID**: Do not use this value for application concepts such as deposit IDs, withdrawal IDs, transfer IDs, order IDs, or payment IDs. Store those as event data, tags, or read-model columns.
...
`CommandExecutor` supports a caller-controlled `commandId UUID` that detects duplicates
**before the handler runs** — no event projections, no advisory locks:

```java
// controller / application service
UUID commandId = parseUuid(request.getHeader("Idempotency-Key")); // must be a UUID
...
The executor inserts the command record using `commandId` as the primary key (`ON CONFLICT
(command_id) DO NOTHING`). If a committed record with that ID already exists,
`rows_affected == 0` and the handler is not invoked. If the transaction rolls back (handler
exception, concurrency violation), the inserted row rolls back atomically — the ID is
released and the next attempt proceeds as new.

**Key rules:**

- UUID v7 is recommended: time-ordered, B-tree-friendly, and lexicographically sortable.
  Any UUID version works for correctness.
- The `commandId` must be generated by the caller and survive across retries — do not
  generate it inside the command object itself.
- Requires `crablet.eventstore.persist-commands=true` (the default). An
  `InvalidCommandException` is thrown if persistence is disabled and a `commandId` is set.
...
// correlation only (no idempotency)
commandExecutor.execute(command, CommandExecutionOptions.builder()
        .correlationId(correlationId)
        .build());
```
...
Use command-level for HTTP APIs and message consumers where the caller controls the ID.
Use event-level (`.idempotent(...)`) when the key lives inside the domain model (e.g.
`deposit_id` on a `DepositMade` event) and the handler must always run to produce it.
...
When `commandId` is set in `CommandExecutionOptions`, the row is written at the **start** of
the transaction — before the handler runs. `ON CONFLICT (command_id) DO NOTHING` detects
committed duplicates. For normal executions (no `commandId`), the row is written after the
append using a server-generated UUID; duplicates are not detected at this level.

Application/business transaction IDs should remain in command data, event data, tags, or read models.
