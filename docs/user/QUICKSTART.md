...
The runnable local reference is the wallet example app. Use it when you want
...
the wallet reference app, and build `crablet-codegen`. Your application lives in a
separate Spring Boot project outside this repo (for example, a sibling `../my-service`
created by copying `templates/crablet-app` or by running `crablet-codegen init`).
