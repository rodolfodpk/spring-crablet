...
**Run this plan before** [`k8s-generation-from-event-model.md`](k8s-generation-from-event-model.md) K8s doc work: it gives a cleaner doc tree (`docs/user/MODULES.md`, unified index) for cross-links from generated K8s docs and `DEPLOYMENT_TOPOLOGY.md`.

## Implementation steps

1. **Add `docs/user/MODULES.md`**
   ...
   - **Link only (do not copy prose):** for poller-backed module **deployment constraints**, in **`docs/user/MODULES.md`** use same-directory form: `[Deployment Topology](DEPLOYMENT_TOPOLOGY.md)`. One line is enough; do not duplicate the topology rules in `MODULES.md`.
...
2. **Slim `README.md`**
   ...
   - **Modules**: 2–3 sentences + **link to [Module reference](docs/user/MODULES.md)**; remove inlined tables.
   - **DCB At A Glance**: keep the small 3-row table + links to `crablet-eventstore/docs/`.
   - **License**: unchanged.

3. **Single documentation index**
   - Combine early navigation and **Where To Go Next** into one section, e.g. `## Documentation`, with subsections **Start**, **Build & operate**, **Architecture** — each 3–6 links max. In **`README.md` (repo root)**, add **`docs/user/PUBLIC_API.md`** (public HTTP / command API; when to use custom controllers) and **`docs/user/MODULES.md`** so links stay root-relative and correct on GitHub.

4. **Contributors (optional, one line in README)**

   - From **repo root `README.md`**, use: `[Build](docs/user/BUILD.md)` and `[CLAUDE.md](CLAUDE.md)` — not `../` paths (those are wrong for a root `README.md` on GitHub).
...
- `docs/user/MODULES.md` contains the full former module tables + prose; no information loss vs previous README; **topology** is link-only, not duplicated.
- `docs/user/PUBLIC_API.md` is linked from the new documentation index.
- All `README.md` and `#anchor` references from `docs/user/` and other consumers resolve after the change.
...
- **[`docs/user/PUBLIC_API.md`](../../user/PUBLIC_API.md)** exists; the README index should link to it (no “future” placeholder).
- **[`k8s-generation-from-event-model.md`](k8s-generation-from-event-model.md)** (K8s codegen): after this refresh lands, that plan adds **`docs/user/MODULES.md`** to its documentation story so generated K8s is discoverable from the module doc; it keeps **`docs/user/DEPLOYMENT_TOPOLOGY.md`** platform-agnostic and links in.
