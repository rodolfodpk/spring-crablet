...
- `root README.md` line 43 already says "The template's `.claude/settings.json` wires
  `crablet-codegen` as an MCP server. You never call `java -jar` directly."
- `templates/crablet-app/CLAUDE.md` already has all behavioral rules (ask for facts,
  update model first, run `crablet_plan`, wait for approval).
- `crablet-codegen/README.md` already documents MCP mode, the exposed tools, and
  `.claude/settings.json` discovery.

**The actual gaps** were in the **template READMEs** and **`templates/crablet-app/CLAUDE.md`**
(missing or misleading MCP and CLI framing), not in the root README or `crablet-codegen` docs.
...
3. **MCP output path** — `crablet_generate` defaults **`output` to `.`** (project root), while
   `make generate` runs `generate --output src/main/java`. Docs must state that Claude should call
   **`crablet_generate` with `output: "src/main/java"`** for this template, or generated sources
...
- Update the short workflow at the top of the README so the `crablet_generate` step also says
...
- `.claude/settings.json` starts `java -jar ./tools/crablet-codegen.jar --mcp` when
  Claude Code opens the project.
- This exposes `crablet_plan`, `crablet_generate`, `crablet_init`, and `crablet_k8s` as
  MCP tools Claude Code calls automatically.
- In the recommended Claude Code workflow, users do not run `java -jar` directly.
- **`crablet_generate` must be called with `output` set to `src/main/java`** — same as
...
- **Local commands:** `make plan` and `make verify` are suitable for **scripted checks** and
  **CI** (plan is deterministic, no Anthropic; verify is Maven). `make generate` is for **manual
  ...
  `crablet_generate`), or for **debugging** — not the default interactive path and not a typical
  CI step. **`make k8s`** matches MCP **`crablet_k8s`**: **deterministic** Kubernetes manifest
...
Include that **`crablet_generate` uses `output: src/main/java`** in this template. Manual section:
`make plan` / `make verify` = CI- or script-friendly; `make generate` = intentional AI regeneration
(same pipeline as `crablet_generate`), not standard CI; list **`make k8s`** alongside
**`crablet_k8s`** (no Anthropic).
...
Require **`crablet_generate` with `output` `src/main/java`** in the feature-slice steps (MCP
...
| `crablet-codegen/README.md` | MCP section already complete; default `output` is documented there |
...
- **`templates/crablet-app/README.md`** — "The workflow" opener lists **`crablet_generate` with
  `output: "src/main/java"`**; added **How code generation works** (`.claude/settings.json` JSON,
  tool list, **`crablet_generate` default vs `src/main/java`**, primary vs local); reframed
  **Local commands**; documented **`make k8s` ↔ `crablet_k8s`** (deterministic, no Anthropic).
- **`templates/README.md`** — **Workflow (primary)** is MCP-first with **`crablet_generate` +
...
- **`templates/crablet-app/CLAUDE.md`** — Step instructs **`crablet_generate` with `output`
...
# crablet_generate output path documented (matches make generate)
...
# make k8s / crablet_k8s mapping documented
rg "crablet_k8s|make k8s" templates/crablet-app/README.md templates/README.md
# grep -E "crablet_k8s|make k8s" templates/crablet-app/README.md templates/README.md
...
**first** steps must be **Open Claude Code** / `event-model.yaml` / **`crablet_*`** — not
...
| **Workflow (primary)** in `templates/README.md` leads with `Open Claude Code` / MCP / `crablet_*`, not `make plan` first | **Pass** — `make plan` only under **Manual or scripted** |
| CI / manual / regeneration wording in both template READMEs | **Pass** |
| `crablet_k8s` and `make k8s` in both template READMEs | **Pass** — **Kubernetes** paragraph in `crablet-app`; `make k8s` in `templates` manual list |
| **Manual read** of **Workflow (primary)** in `templates/README.md` | **Pass** — does not start with `make plan` / `make generate` |
| **The workflow** code block at top of `templates/crablet-app/README.md` includes `output` for `crablet_generate` | **Pass** — aligned with **How code generation works** |
