...
@DisplayName("EventPollerObservationAutoConfiguration")
class EventPollerObservationAutoConfigurationTest {

    private final ApplicationContextRunner runner = new ApplicationContextRunner()
            .withConfiguration(AutoConfigurations.of(EventPollerObservationAutoConfiguration.class));
    ...
    void registersListenerWhenObservationRegistryPresent() {
        runner
                ...
                .run(ctx -> assertThat(ctx).hasSingleBean(EventPollerObservationListener.class));
    }

    @Test
    @DisplayName("listener is absent when no ObservationRegistry bean is present")
    void noListenerWhenNoObservationRegistry() {
        runner.run(ctx -> assertThat(ctx).doesNotHaveBean(EventPollerObservationListener.class));
    }
    ...
    void userBeanTakesPrecedence() {
        runner
                ...
                .run(ctx -> {
                    assertThat(ctx).hasSingleBean(EventPollerObservationListener.class);
                    assertThat(ctx.getBean(EventPollerObservationListener.class))
    ...
    static class WithCustomListener {
        @Bean("customListener")
        EventPollerObservationListener customListener(ObservationRegistry registry) {
            return new EventPollerObservationListener(registry);
        }
    }
}
