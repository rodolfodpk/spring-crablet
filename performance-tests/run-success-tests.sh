...
echo "🚀 Starting Success Suite Performance Tests..."
...
seed_data() {
  echo "🌱 Seeding success suite data..."
  "${K6_BIN}" run "${PERF_TESTS_DIR}/setup/seed-success-data.js"
  echo "✅ Success suite data seeding complete."
}
...
# Define success tests to run
tests=(
    "transfer-success.js:Transfer Success Test"
    "mixed-workload-success.js:Mixed Workload Success Test"
    "spike-success.js:Spike Success Test"
)

# Run each test
for test_entry in "${tests[@]}"; do
    IFS=':' read -r script_name display_name <<< "${test_entry}"
    run_k6_test "${script_name}" "${display_name}"
done

echo "🎉 Success suite performance tests completed successfully!"
